...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("eyarod-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("eyarod-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("HSATD-SYNCER",
		"Eiyaro",
	...
	syncedCmd, err := common.StartCmd("HSATD-SYNCED",
		"Eiyaro",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
