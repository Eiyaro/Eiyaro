...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("eyarod-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("eyarod datadir: %s", dataDir)
	...
	spawn("startNode-htndCmd.Wait", func() {
		...
		if err != nil {
			...
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes eyarod closes ungracefully
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	hoosatMinerCmd, err := common.StartCmd("MINER",
		"hoosatminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("hoosat-miner-Cmd.Wait", func() {
		err := hoosatMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(hoosatMinerCmd, "hoosatMinerCmd")
	return nil
}
