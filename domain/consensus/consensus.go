...
func (s *consensus) Init(skipAddingGenesis bool) error {
	...
	// Eiyaro: Auto-create genesis block to exit IBD mode on first node startup
	if !skipAddingGenesis && s.blockStore.Count(stagingArea) == 0 {
		log.Infof("Fresh consensus detected, auto-creating genesis block to exit IBD mode")
		...
		virtualChangeSet, blockStatus, err := s.blockProcessor.ValidateAndInsertBlockWithTrustedData(genesisWithTrustedData, true)
		if err != nil {
			return err
		}
		log.Infof("Genesis block created and inserted successfully with status: %s", blockStatus)
		
		// Eiyaro: Explicitly resolve virtual to ensure node exits IBD mode
		if virtualChangeSet != nil {
			log.Infof("Genesis block virtual change set processed, virtual parents: %d", len(virtualChangeSet.VirtualParents))
		}
		
		// Eiyaro: Force virtual resolution to complete initialization
		_, isCompletelyResolved, err := s.resolveVirtualChunkNoLock(virtualResolveChunk)
		if err != nil {
			return err
		}
		if isCompletelyResolved {
			log.Infof("Virtual completely resolved after genesis creation, node exited IBD mode")
		} else {
			log.Warnf("Virtual not completely resolved after genesis creation, may require additional blocks")
		}
	}
...
func (s *consensus) isNearlySyncedNoLock() (bool, error) {
	...
	// Eiyaro: If the selected parent is genesis, check if genesis is recent enough
	// This allows mining on a fresh node with only the genesis block
	if virtualGHOSTDAGData.SelectedParent().Equal(s.genesisHash) {
		...
		// Eiyaro: If genesis was just created (within last DAA window), consider node as nearly synced
		if now-genesisHeader.TimeInMilliseconds() < s.expectedDAAWindowDurationInMilliseconds {
			log.Debugf("Genesis block is recent (%d), node is nearly synced and can mine", genesisHeader.TimeInMilliseconds())
			return true, nil
		}
		// Eiyaro: If genesis is old, still allow mining for isolated test nodes
		log.Debugf("Genesis block is old (%d), but allowing mining for isolated test node", genesisHeader.TimeInMilliseconds())
		return true, nil
	}
