...
func (bp *blockProcessor) validateAndInsertBlock(stagingArea *model.StagingArea, block *externalapi.DomainBlock,
	...
	log.Debug(logger.NewLogClosure(func() string {
		...
		// If virtual GHOSTDAG data does not exist yet (e.g., fresh staging consensus without genesis during IBD),
		// skip logging instead of failing the whole flow.
		if database.IsNotFoundError(err) {
...
func (bp *blockProcessor) loadUTXODataForGenesis(stagingArea *model.StagingArea, block *externalapi.DomainBlock) {
	...
	// found here for anyone to verify: https://github.com/Eiyaro/Eiyaro/blob/dbf18d8052f000ba0079be9e79b2d6f5a98b74ca/domain/consensus/processes/blockprocessor/resources/utxos.gz
