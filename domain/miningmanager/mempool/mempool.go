...
import (
	...
	miningmanagermodel "github.com/Eiyaro/Eiyaro/domain/miningmanager/model"
)
...
func (mp *mempool) BlockCandidateTransactions() []*externalapi.DomainTransaction {
	...
	for i := range readyTxs {
		...
		if !hasCoinbaseInput && numExtraOuts > 2 && readyTxs[i].Fee < uint64(numExtraOuts)*constants.SeyPerEY {
			log.Debugf("Filtered spam tx %s", consensushashing.TransactionID(readyTxs[i]))
			continue
		}

		if hasCoinbaseInput || readyTxs[i].Fee > checkedUint64FromExtraOutputs(numExtraOuts)*constants.SeyPerEY {
			candidateTxs = append(candidateTxs, readyTxs[i])
		} else {
