...
// Outpoint defines a hoosat data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new hoosat transaction outpoint point with the
...
// TxIn defines a hoosat transaction input.
type TxIn struct {
...
// NewTxIn returns a new hoosat transaction input with the provided
...
// TxOut defines a hoosat transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new hoosat transaction output with the provided
...
// MsgTx implements the Message interface and represents a hoosat tx message.
