...
func checkNodeSyncStatus(context *rpccontext.Context) error {
	...
	// Eiyaro: Allow isolated nodes (no peers) to submit blocks if nearly synced
	...
	// Eiyaro: If no peers but nearly synced, allow block submission
	// This handles the case of a fresh node with only genesis block
	if !hasPeers && isNearlySynced {
		log.Debugf("Isolated node is nearly synced, allowing block submission")
		return nil
	}

	// Traditional check: must have peers and be nearly synced
	if !hasPeers {
