...
const (
	defaultConfigFilename      = "eiyarod.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "eiyarod.log"
	defaultErrLogFilename      = "eiyarod_err.log"
...
var (
	// DefaultAppDir is the default home directory for eiyarod.
	DefaultAppDir = util.AppDir("eiyaro", false)
...
// Flags defines the configuration options for htnd.
//
...
type Flags struct {
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 42420, testnet: 16210)"`
	...
	PruningIntervalHours            uint64        `long:"pruning-interval-hours" description:"Minimum number of hours post pruning point movement that the UTXO recalculation will be triggered, alongside the block deletion. 0 (default) prunes at every pruning point update using standard rules."`
...
// Config defines the configuration options for htnd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default htnd configuration
...
func LoadConfig() (*Config, error) {
	...
	if cfg.IsArchivalNode && cfg.PruningIntervalHours > 0 {
		str := "%s: archival and pruning-interval-hours cannot be used " +
			"together -- archival mode does not prune"
		err := errors.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		fmt.Fprintln(os.Stderr, usageMessage)
		return nil, err
...
// createDefaultConfig copies the file sample-htnd.conf to the given destination path,
