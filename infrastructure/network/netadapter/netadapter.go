...
import (
	...
	"github.com/Eiyaro/Eiyaro/infrastructure/network/netadapter/server/grpcserver"
...
func NewNetAdapter(cfg *config.Config) (*NetAdapter, error) {
	...
	p2pServer, err := grpcserver.NewEiyaroServer(cfg.Listeners)
	if err != nil {
		return nil, err
	}
	rpcServer, err := grpcserver.NewEiyaroRPCServer(cfg.RPCListeners, cfg.RPCMaxClients)
	if err != nil {
		return nil, err
	}
	adapter := NetAdapter{
