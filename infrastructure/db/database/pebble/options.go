...
func Options(cacheSizeMiB int) *pebble.Options {
	...
	// -
	// Block cache -?aim higher in 2026 (8-?6 GiB realistic)
	// -
	...
	opts := &pebble.Options{
		...
		L0CompactionThreshold:     getEnvInt("HTND_L0_COMPACTION_THRESHOLD", 64),      // was 16 -?start here, try up to 128-?56 if still building up
		L0StopWritesThreshold:     getEnvInt("HTND_L0_STOP_WRITES_THRESHOLD", 200),    // was 48 -?must be >= L0CompactionThreshold; 200-?00 range common in heavy-ingest
