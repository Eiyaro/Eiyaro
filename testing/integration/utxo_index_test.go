...
func TestUTXOIndex(t *testing.T) {
	...
	// Setup a single eyarod instance
	...
	eyarod, teardown := setupHarness(t, harnessParams)
	defer teardown()

	// skip the first block because it's paying to genesis script,
	// which contains no outputs
	mineNextBlock(t, eyarod)

	coinSupplyBefore, err := eyarod.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error retrieving coin supply before mining: %s", err)
	}
	blockCountBefore, err := eyarod.rpcClient.GetBlockCount()
	if err != nil {
	...
	err = eyarod.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	coinSupplyAfter, err := eyarod.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error retrieving coin supply after mining: %s", err)
	}
	blockCountAfter, err := eyarod.rpcClient.GetBlockCount()
	if err != nil {
	...
	for i := range transactionAmountToSpend {
		...
		_, err = eyarod.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := eyarod.rpcClient.GetUTXOsByAddresses([]string{miningAddress1}, 0)
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixEiyaroSim)
	if err != nil {
