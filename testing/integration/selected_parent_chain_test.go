...
func TestVirtualSelectedParentChain(t *testing.T) {
	...
	// Setup a couple of eyarod instances
	eyarod1, eyarod2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := eyarod1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In eyarod2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for range blockAmountToMine + 1 {
		chain2Tip = mineNextBlock(t, eyarod2)
	}

	// Connect the two eyarods. This should trigger sync
	// between the two nodes
	connect(t, eyarod1, eyarod2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := eyarod1.rpcClient.GetVirtualSelectedParentChainFromBlock(
