...
// 2. A wrapper that implements isEiyaroMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `EiyaroMessage_` prefix and `Request` suffix
...
// generateEiyaroMessage generates a wrapped EiyaroMessage with the given `commandValue`
func generateEiyaroMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.EiyaroMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	EiyaroMessage := reflect.New(reflect.TypeFor[protowire.EiyaroMessage]())
	EiyaroMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return EiyaroMessage.Interface().(*protowire.EiyaroMessage), nil
}
