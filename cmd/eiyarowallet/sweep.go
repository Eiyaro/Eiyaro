...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libeiyarowallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libeiyarowallet.EiyarowalletdUTXOsTolibeiyarowalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatEY(splitTransactions[i].Outputs[0].Value), " EY")
		totalExtracted += splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatEY(totalExtracted), " EY")
