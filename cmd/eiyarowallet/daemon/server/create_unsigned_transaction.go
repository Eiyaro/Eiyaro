...
const minChangeTarget = constants.SeyPerEY / 5
...
func (s *server) createUnsignedCompoundTransaction(address string, fromAddressesString []string, useExistingChangeAddress bool, limit uint32) ([][]byte, error) {
	...
	selectedUTXOs, _, changeSey, err := s.selectCompoundUTXOs(feePerInput, fromAddresses)
	if err != nil {
		return nil, err
	}
	...
	unsignedTransaction, err := libeiyarowallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectCompoundUTXOs(feePerInput int, fromAddresses []*walletAddress) (
	selectedUTXOs []*libeiyarowallet.UTXO, totalReceived uint64, changeSey uint64, err error,
) {
	selectedUTXOs = make([]*libeiyarowallet.UTXO, 0, targetCompoundInputs)
	...
	if totalValue <= fee {
		return nil, 0, 0, errors.Errorf("not enough funds: total %d sey < fee %d sey", totalValue, fee)
	}

	changeSey = totalValue - fee
	log.Infof("Compounding %d EY and paying %f fee", changeSey/100_000_000, float64(fee)/float64(100_000_000))

	return selectedUTXOs, totalValue, changeSey, nil
}
...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool, payload []byte, limit uint32) ([][]byte, error) {
	...
	selectedUTXOs, spendValue, changeSey, err := s.selectUTXOs(amount, isSendAll, feePerInput, fromAddresses)
	if err != nil {
		return nil, err
	}
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feePerInput uint64, fromAddresses []*walletAddress) (
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.SeyPerEY, float64(totalValue)/constants.SeyPerEY)
	}
...
func selectedUTXOsContain(selectedUTXOs []*libeiyarowallet.UTXO, utxo *walletUTXO) bool {
