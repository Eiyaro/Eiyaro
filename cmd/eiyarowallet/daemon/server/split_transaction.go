...
import (
	...
	"github.com/Eiyaro/Eiyaro/domain/miningmanager/mempool"
...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libeiyarowallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libeiyarowallet.UTXO{
	...
	payments := []*libeiyarowallet.Payment{{
		Address: toAddress,
		Amount:  sentValue,
	}}
	if totalValue > sentValue {
		payments = append(payments, &libeiyarowallet.Payment{
			Address: changeAddress,
			Amount:  totalValue - sentValue,
		})
	}

	mergeTransactionBytes, err := libeiyarowallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by htnwallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libeiyarowallet.UTXO, 0, endIndex-startIndex)
	totalSey := uint64(0)
	...
	for i := startIndex; i < endIndex && i < len(transaction.PartiallySignedInputs); i++ {
		...
		selectedUTXOs = append(selectedUTXOs, &libeiyarowallet.UTXO{
		...
		totalSey += selectedUTXOs[i-startIndex].UTXOEntry.Amount()
		totalSey -= feePerInput
	}
	unsignedTransactionBytes, err := libeiyarowallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := libeiyarowallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libeiyarowallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*libeiyarowallet.UTXO, totalValueAdded uint64, err error,
) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libeiyarowallet.UTXO{
