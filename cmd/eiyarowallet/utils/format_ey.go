...
// FormatEY takes the amount of sey as uint64, and returns amount of EY with 8 decimal places
func FormatEY(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SeyPerEY)
	}
	return res
}

// EYToSey takes in a string representation of the EY value to convert to Sey
func EYToSey(amount string) (uint64, error) {
	err := validateEYAmountFormat(amount)
	if err != nil {
		return 0, err
	}
	...
	if constants.SeyPerEY%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sey when SeyPerEY is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SeyPerEY))
	decimalStr := ""
...
func validateEYAmountFormat(amount string) error {
