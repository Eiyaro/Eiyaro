...
// Takes in a string representation of the EY value to convert to Sey
func TestEYToSey(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := EYToSey(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := EYToSey(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	validCases := []string{
		...
		"184467440737.09551615", // Maximum input that can be represented in sey later
		"184467440737.09551616", // Cannot be represented in sey, but we'll acccept for "correct format"
		"999999999999.99999999", // Cannot be represented in sey, but we'll acccept for "correct format"
	}

	for _, testCase := range validCases {
		err := validateEYAmountFormat(testCase)
		if err != nil {
			t.Error(err)
		}
	}
